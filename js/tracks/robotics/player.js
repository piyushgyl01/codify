/** The session runner: the daily mission's check, practice sets and boss fights. */
import { S } from '../../state.js';
import {
  currentQuestion, answer, sessionOver, finishSession, abandonSession,
  startMission, startPractice, startBoss, bossReady, timeLimit, markShown, skillEntry,
} from './actions.js';
import { skillById } from './skills.js';
import { monthByN } from './roadmap.js';
import { bossFor, BOSS_HP, BOSS_HEARTS, BOSS_QUESTIONS } from './bosses.js';
...
export function openMission(rerender) {
  if (S.active && S.active.mode !== 'mission' && !S.active.mission) { toast('Finish the session you already started first.'); return run(rerender); }
  if (!startMission()) { toast('Today\'s mission is done. The next one opens tomorrow.'); return; }
...
function nextLine(g, day) {
  const e = skillEntry(g.skill), n = roundFor(g.to).n, when = relDays(daysBetween(day, e?.due || day));
...
function run(rerender) {
  ...
  function progressStrip(a) {
    ...
    if (a.mode !== 'boss') {
      ...
      const clockBar = a.mode === 'mission' && phase === 'ask'
        ? `<div class="pl-timebox"><div class="pl-clock"><i data-clock></i></div><div class="tiny num" data-left></div></div>` : '';
      return `<div class="pl-segs">${segs}</div>${a.mode === 'mission' ? clockBar : combo}`;
    }
  ...
  function feedback(a, r) {
    ...
    const round = r.round ? `<div class="round-res ${r.round.move > 0 ? 'up' : r.round.move < 0 ? 'down' : ''}">
        ...
        <div class="tiny" style="margin-top:2px">${esc(nextLine(r.round, a.day))}</div></div>` : '';
  ...
  function wire(a) {
    el.querySelector('[data-x]').onclick = () => {
      if (a.mode === 'practice') { clearInterval(tick); close(); rerender(); toast('Paused — pick it up from Today or Robots.'); return; }
      if (a.mode === 'mission') {
        dialog(`<div class="h2">Pause the mission?</div>
  ...
  function celebrate(s) {
    if (!s) return;
    const big = (s.mode === 'mission' && (s.perfect || s.groups.some(g => g.move > 0))) || (s.mode === 'boss' && s.won);
